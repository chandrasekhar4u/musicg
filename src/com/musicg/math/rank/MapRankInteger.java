...
public class MapRankInteger implements MapRank {
	...
	public MapRankInteger(final Map<?, Integer> map, final boolean acsending) {
		this.map = map;
	...
	public List getOrderedKeyList(int numKeys, final boolean sharpLimit) { // if sharp limited, will return sharp numKeys, otherwise will return until the values not equals the exact key's value
		...
		if (map.size() > 0) {
			...
			final int passValue = getOrderedValue(array, targetindex); // this value is the value of the numKey-th element
			// get the passed keys and values
			final Map passedMap = new HashMap();
			final List<Integer> valueList = new LinkedList<Integer>();
			mapIterator = mapEntrySet.iterator();

			while (mapIterator.hasNext()) {
				final Entry entry = mapIterator.next();
				final int value = (Integer) entry.getValue();
			...
			// sort the value list
			final Integer[] listArr = new Integer[valueList.size()];
			...
			while (true) {
				final int targetValue = listArr[index];
	...
	private int getOrderedValue(final int[] array, final int index) {
		locate(array, 0, array.length - 1, index);
		return array[index];
	}

	// sort the partitions by quick sort, and locate the target index
	private void locate(final int[] array, final int left, final int right, final int index) {
