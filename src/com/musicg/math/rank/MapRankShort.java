...
public class MapRankShort implements MapRank {
	...
	public MapRankShort(final Map<?, Short> map, final boolean acsending) {
		this.map = map;
	...
	public List getOrderedKeyList(int numKeys, final boolean sharpLimit) { // if sharp limited, will return sharp numKeys, otherwise will return until the values not equals the exact key's value
		...
		if (map.size() > 0) {
			...
			final short[] array = new short[map.size()];
			int count = 0;
			...
			final short passValue = getOrderedValue(array, targetindex); // this value is the value of the numKey-th element
			// get the passed keys and values
			final Map passedMap = new HashMap();
			final List<Short> valueList = new LinkedList<Short>();
			mapIterator = mapEntrySet.iterator();

			while (mapIterator.hasNext()) {
				final Entry entry = mapIterator.next();
				final short value = (Short) entry.getValue();
			...
			// sort the value list
			final Short[] listArr = new Short[valueList.size()];
			...
			while (true) {
				final short targetValue = listArr[index];
	...
	private short getOrderedValue(final short[] array, final int index) {
		locate(array, 0, array.length - 1, index);
		return array[index];
	}

	// sort the partitions by quick sort, and locate the target index
	private void locate(final short[] array, final int left, final int right, final int index) {
	...
	private void swap(final short[] array, final int i, final int j) {
